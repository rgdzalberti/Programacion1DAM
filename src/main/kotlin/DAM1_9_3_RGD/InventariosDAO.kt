...
class InventariosDAO(val c: Connection) {
    ...
    companion object {
        ...
        private const val INDEX_INVENTARIOS = "CREATE INDEX IDX_ID_TIENDA ON INVENTARIOS(ID_TIENDA)"
    }
    ...
    fun insert(inventario: Inventarios) {
        ...
        try {
            c.prepareStatement(INSERT_INVENTARIOS_SQL).use { st ->
                ...
                if (inventario.Precio >= 2000) {st.setDouble(4, inventario.Precio * 1.15)} else st.setDouble(4, inventario.Precio)
